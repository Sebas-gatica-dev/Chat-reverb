...
',
        ...
        [
            ...
            'links' => [
                [
                    ...
                    'url' => route('panel.settings.budgets.budgetems.list'),
                    'active' => 'panel.settings.budgets.budgetems.*',
                    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
                                <path d="M..." />
                              </svg>',
                ],
                [
                    ...
                    'url' => route('panel.settings.budgets.template.list'),
                    'active' => 'panel.settings.budgets.template.*',
                    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
                            <path d="M..." />
                          </svg>',
                ],
                ...
                [
                    ...
                    'url' => route('panel.settings.budgets.documents.list'),
                    'active' => 'panel.settings.budgets.documents.*',
                    'icon' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
                            <path d="M..." />
                          </svg>',
                ],
        ...
        [
            ...
            'url' => route('panel.settings.forms.list'),
            'active' => 'panel.settings.forms.*',
            'icon' => '<svg xmlns="http://www.w3.org/2000/svg" fill="none" class="h-6 w-6 shrink-0" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        [
            'title' => 'Plantillas de estadísticas',
            'url' => route('panel.settings.stats.list'),
            'active' => 'panel.settings.stats.*',
            'icon' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-6 w-6 shrink-0">
...
',
        ...
        [
            'title' => 'Salarios',
            'url' => route('panel.settings.salaries.list'),
            'active' => 'panel.settings.salaries.*',
            'icon' => '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
        ...
        @foreach ($mainLinks as $link)
            @if (Gate::allows('access-function', $link['gate']))
                @if (isset($link['links']) && is_array($link['links']) && count($link['links']) > 0)
                    <!-- Dropdown Acordeón -->
                    <li class="w-full">
                        <div x-data="{ open: {{ collect($link['links'])->contains(fn($subLink) => request()->routeIs($subLink['active'])) ? 'true' : 'false' }} }" class="relative">
                            <button @click="open = !open"
                                class="group flex items-center w-full gap-x-3 rounded-md py-2 pl-2 pr-3 text-sm font-semibold text-gray-700 hover:text-indigo-600 hover:bg-gray-50 focus:outline-none">
                                {!! $link['icon'] !!}
                            ...
                            <div x-show="open" x-transition class="hidden lg:block mt-1 space-y-1">
                                @foreach ($link['links'] as $subLink)
                                    @if (Gate::allows('access-function', $subLink['gate'] ?? true))
                                        <a href="{{ $subLink['url'] }}" wire:navigate
                                            class="group flex gap-x-3 rounded-md py-2 pl-2 pr-3 text-sm leading-6 font-semibold {{ request()->routeIs($subLink['active']) ? 'bg-gray-100 text-indigo-600' : 'text-gray-700 hover:text-indigo-600 hover:bg-gray-50' }}">
                                            {!! $subLink['icon'] !!}
