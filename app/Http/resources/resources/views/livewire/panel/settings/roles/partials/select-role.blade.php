<div>
    ...
    <label for="role" class="block text-sm font-medium leading-6 text-gray-900">Copiar funciones del rol:</label>
    ...
    <div class="mt-2">
        <div x-data="{ open: false }" @click.away="open = false" @keyup.escape= "open = false" class="relative mb-4">
            <button @click="open = !open" type="button"
                ...
                <span class="block truncate text-sm"> {{ $selectedRole ?  $selectedRole->name : 'Seleccione un rol' }} </span>
            ...
            <div x-show="open" class="absolute z-10 w-full bg-white rounded-md shadow-lg" x-cloak>
                <input type="text" wire:model.live.debounce.300ms="searchTerm" placeholder="Buscar rol..."
                ...
                <ul class="max-h-60 w-full overflow-auto py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
                    ...
                    @forelse ($roles as $role)
                        <li class="text-gray-900 text-sm relative cursor-default select-none py-2 pl-8 pr-4"
                            id="listbox-option-{{ $role['id'] }}" role="option">
                            <button wire:click.prevent="selectRole('{{ $role['id'] }}')" class="w-full text-left" @click="open = false" >
                                <span class="font-normal block truncate">{{ $role['name'] }}</span>
