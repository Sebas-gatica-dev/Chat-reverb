<div>
    <label for="module" class="block text-sm font-medium leading-6 text-white">Modulo</label>
    ...
    <div class="mt-2">
        <div x-data="{ open: false }" @click.away="open = false" @keyup.escape= "open = false" class="relative mb-4">
            <button @click="open = !open" type="button"
                ...
                <span class="block truncate text-sm"> {{ $selectedModule ?  $selectedModule->name : 'Seleccione un modulo' }} </span>
            ...
            <div x-show="open" class="absolute z-10 mt-1 w-full bg-white rounded-md shadow-lg">
                <input type="text" wire:model.live.debounce.300ms="searchTerm" placeholder="Buscar modulo..."
                ...
                <ul class="max-h-60 w-full overflow-auto py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm"
                    ...
                    @forelse ($modules as $module)
                        <li class="text-gray-900 text-sm relative cursor-default select-none py-2 pl-8 pr-4"
                            id="listbox-option-{{ $module['id'] }}" role="option">
                            <button wire:click.prevent="selectModule('{{ $module['id'] }}')" class="w-full text-left" @click="open = false" >
                                <span class="font-normal block truncate">{{ $module['name'] }}</span>
                                @if (isset($selectedModule) && $selectedModule->id == $module['id'])
