...
class Business extends Model
{
    ...
    {
        return $this->hasOne(Subscription::class)->whereIn('status', [1, 3])->latest('created_at');
    }


    public function paymentPending()
    {
        return $this->hasOne(Payment::class)->where('status', '!=', 1)->latest();
    }
    ...
    {
        return $this->hasManyThrough(Payment::class, Subscription::class)->orderBy('payments.created_at', 'desc');
    }
    ...
    public function getCombinedFunctions()
    {
        ...
        return Cache::remember('business-functions-' . $this->id. '-user-'. auth()->user()->id, now()->addHours(1), function () {
            ...
            $planFunctions = $this->subscriptionActive->plan->features()->select(['id', 'slug', 'count'])->get();
            }else{
                $planFunctions = Plan::where('name', 'Plan Básico')->first()->features()->select(['id', 'slug', 'count'])->get();
                //esto es para que cuando no tenga ningun plan activo ponerle un plan basico, en realidad deberia haber un plan en donde solo vengan las funciones basicas
            }
