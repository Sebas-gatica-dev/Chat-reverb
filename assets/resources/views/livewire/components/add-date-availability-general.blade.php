<div>
    ...
    <h3 class="mb-4 text-base">Agregar disponibilidad horaria</h3>
    ...
    @if ($groupedAvailabilities)
        <div
            class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 gap-x-4 border-t border-gray-200 pt-6 border-dashed gap-y-4 xl:gap-y-0">
            @foreach ($groupedAvailabilities as $day => $availabilities)
                <div>
                    <span class="font-semibold">{{ $this->translateDay($day) }}:</span>
                    <div class="flex flex-wrap gap-2 mt-2">
                        @foreach ($availabilities as $availability)
                            <div
                                class="flex items-center gap-2 p-2 border border-gray-300 rounded-md text-sm w-full justify-between">
                                <span class="text-[13.5px]">{{ $availability['start_time'] }} -
