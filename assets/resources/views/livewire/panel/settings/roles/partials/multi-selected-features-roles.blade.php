                    ...
                    <nav class="-mb-px flex space-x-8 border-b border-gray-200  w-full" aria-label="Tabs">
                        ...
                        @forelse ($modules as $module)
                            ...
                            <button wire:click="changeModule('{{ $module['id'] }}')" wire:key="{{ $module['id'] }}"
                                class="group inline-flex items-center border-b-2
                    px-1 py-4 text-sm font-medium
                    {{ $module['id'] == $selectModule ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700' }}
                    ">
                    ...
                    <div class="overflow-x-auto">
                        <nav class="-mb-px flex space-x-8 w-full" aria-label="Tabs">
                            @forelse ($modules as $module)
                                <button wire:click="changeModule('{{ $module['id'] }}')" wire:key="{{ $module['id'] }}"
                                    class="group inline-flex items-center border-b-2
                            px-1 py-4 text-sm font-medium flex-shrink-0
                            {{ $module['id'] == $selectModule ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700' }}
                            ">
                                    <span
                                        class="group-hover:text-gray-500 {{ $module['id'] == $selectModule ? 'text-indigo-500' : 'text-gray-400' }}">
                                        {{ $module['name'] }}
                                    </span>
                                </button>
                            @empty
