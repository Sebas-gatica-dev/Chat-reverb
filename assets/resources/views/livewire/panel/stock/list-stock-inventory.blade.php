...
<div>
  ...
  <div class="border-t border-gray-200 md:py-10 py-5 col-span-3 bg-white shadow-sm" id="disclosure-1"
      ...
      <div
          ...
          @dump($startEntryDateRange)
          @dump($endEntryDateRange)
          @dump($startExpirationDateRange)
          @dump($endExpirationDateRange)
              ...
              <div class="inline-block min-w-full py-6 align-middle md:px-6 lg:px-8">
                  <table
                      class="min-w-full divide-y divide-gray-300 bg-white ring-1 ring-black ring-opacity-5 md:rounded-lg md:shadow">
                        ...
                        <thead class="">
                            <tr>
                                @foreach(['Tag', 'Fecha de ingreso', 'Fecha vencimiento', 'Lote', 'Costo', 'Ganancia', 'Status', 'Valor actual', 'Deposito', 'Acciones'] as $header)
                            ...
                            <tr wire:key="{{ $unit->id }}" class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 rounded-lg">
                                    <div class="flex items-center">
                                        ...
                                        {{-- <div x-data="{ copied: false }" class="ml-2">
                                            <button @click="navigator.clipboard.writeText('{{ $unit->tag }}'); copied = true; setTimeout(() => copied = false, 2000)"
                                                ...
                                                <svg x-show="copied" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-green-500" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $unit->cost ? '$' . number_format($unit->cost, 2) : 'no definido' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    @php
                                        $statusColor = UnitsStatusEnum::getColor(intval($unit->status));
                                    @endphp
                                    <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-{{ $statusColor }}-100 text-{{ $statusColor }}-800">
                                        {{ UnitsStatusEnum::getStatus(intval($unit->status)) ?? 'no definido' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{-- @dd($unit->product) --}}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $unit->warehouse->name ?? 'no definido' }}
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">
                                    No hay unidades de inventario disponibles.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
